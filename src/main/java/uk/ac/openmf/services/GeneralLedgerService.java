...
public class GeneralLedgerService {
	...
	public static boolean updateLedger() throws ParseException{
		...
		for (OpenMFChartOfAccounts openMFChartOfAccounts : coas) {
			...
			if(ledgerVal != null){
				if(ledgerVal.getBalanceavailable() == null)
					ledgerVal.setBalanceavailable("0.00");
				if(ledgerVal.getBalancePending() == null)
					ledgerVal.setBalancePending("0.00");
	...
	public static void generateTodaysLedgerForAllCoAs(String forDate){
		...
		for (OpenMFChartOfAccounts openMFChartOfAccount : coas) {
			OpenMFGeneralLedger openMFGeneralLedger = AppContext.getAppContext().getGeneralLedgerManager().newGeneralLedger(OpenMFConstants.FIELD_VALUE_SYSTEM);
			...
			openMFGeneralLedger.setCreatedById(OpenMFConstants.FIELD_VALUE_SYSTEM);
			int val = AppContext.getAppContext().getGeneralLedgerManager().entityCount() + 1;
			openMFGeneralLedger.setGeneralledgerid(GenerateAccountNumber.getAccNumberService().generateLedgerAccNumber(val));
			openMFGeneralLedger.setPostedby(OpenMFConstants.FIELD_VALUE_SYSTEM);
